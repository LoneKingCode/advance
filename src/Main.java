...
public class Main {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        findEnd = false;
        backtrack(0, 0, 0, board, word.toCharArray(), visited);
        return findEnd;
    }
    ...
    private void backtrack(int row, int col, int index, char[][] board, char[] word, boolean[][] visited) {
        ...
        for (int i = row; i <= maxRowIndex; i++) {
            for (int j = col; j <= maxColIndex; j++) {
                //判断当前点是否和其他周围点有接触的
                boolean adjoin = i - 1 >= 0 && visited[i - 1][j]
                        || i + 1 <= maxRowIndex && visited[i + 1][j]
                        || j - 1 >= 0 && visited[i][j - 1]
                        || j + 1 <= maxColIndex && visited[i][j + 1];
                adjoin = (adjoin && index > 0 || index == 0);
                //如果字符相等 并且此点未被使用
                if (board[i][j] == word[index] && !visited[i][j] && adjoin) {
                    System.out.println(index + ":" + word[index]+" i,j" + i+","+j);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new Main().exist(board, "abbbbaababaa"));
